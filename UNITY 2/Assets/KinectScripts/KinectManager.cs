...
//	public bool NearMode = false;
	...
	//to check if ball collided with user's right hand
	void collission(){
	KinectManager manager = KinectManager.Instance;
	if(manager.GetJointPosition(1, 19).x==GeneratedPosition().x &&manager.GetJointPosition(1, 19).y==GeneratedPosition().y&&manager.GetJointPosition(1, 19).z==GeneratedPosition().z){
	Debug.Log("collided");
	}
	}
	//check which foot is front
	void Whichfoot()
	{
	KinectManager manager = KinectManager.Instance;
